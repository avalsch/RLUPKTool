...
namespace RLUPKTool
{
    ...
    public class TArray<T> : List<T>, IUESerializable
        where T : new()
    {
    ...
    public class FCompressedChunkInfo : IUESerializable
    {
        ...
        {
            UncompressedOffset =
                Sum.LicenseeVersion >= 22 ? Reader.ReadInt64() : Reader.ReadInt32();
    ...
    public class FPackageFileSummary : IUESerializable
    {
        ...
        private TArray<FUnknownTypeInSummary> UnknownTypeArray =
            new TArray<FUnknownTypeInSummary>();
        ...
        public void Deserialize(BinaryReader Reader)
        {
            ...
            CompressedChunks = new TArray<FCompressedChunkInfo>(() =>
                new FCompressedChunkInfo(this)
            );
        ...
        private static void ProcessFile(string Path, string OutPath)
        {
            using (var Input = File.OpenRead(Path))
            {
                using (var Reader = new BinaryReader(Input))
                {
                    ...
                    var ChunkInfo = new TArray<FCompressedChunkInfo>(() =>
                        new FCompressedChunkInfo(Sum)
                    );
                    ...
                    using (var DecryptedStream = new MemoryStream(DecryptedData))
                    {
                        using (var DecryptedReader = new BinaryReader(DecryptedStream))
                        {
                            ...
                            DecryptedStream.Seek(
                                Sum.ExportOffset - Sum.NameOffset,
                                SeekOrigin.Begin
                            );
                        }
                    }
